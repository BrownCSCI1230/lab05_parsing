...
### 2.1. Global Data
First, take a look at ```struct CS123SceneGlobalData```:
```cpp
...
### 2.2. Lights
Next, look at ```enum class LightType```:
...
Now, let's take a look at ```struct CS123SceneLightData```:
...
### 2.3. Camera
For the camera, take a look at ```struct CS123SceneCameraData```:
...
### 2.4. Primitives
Moving on to ```enum class PrimitiveType```, we have multiple different types of primitives:
```cpp
...
The material field describes all the important information about what the primitive looks like (its color, how shiny it is, texture, etc). You can look at the ```struct CS123SceneMaterial``` for details, but don't worry if you don't understand any of the terms if they haven't yet been covered in lectures.
